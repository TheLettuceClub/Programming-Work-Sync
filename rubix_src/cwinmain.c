...
//--------------------------------------------------------------------------------------------------
HWND ghwnd;
int xres, yres, mousx, mousy, bstatus = 0, dmousx = 0, dmousy = 0, dmousz = 0, ActiveApp = 1;
char keystatus[256], *prognam = "CWinMain by Ken Silverman";
int shkeystatus = 0;
#define KEYBUFSIZ 256
static int keybuf[KEYBUFSIZ], keybufr = 0, keybufw = 0, keybufw2 = 0;
...
static void wminput_uninit () { if (huser32lib) { FreeLibrary(huser32lib); huser32lib = 0; } }
...
int startdirectdraw (INT_PTR *f, INT_PTR *p, INT_PTR *x, INT_PTR *y)
{
	...
	if ((xres != gcxres) || (yres != gcyres))
	{
		hdc = GetDC(ghwnd);
		DeleteObject(gscr.hBitmap); gcxres = xres; gcyres = yres;
...
{
	if (peekwindowproc) { int i = peekwindowproc(hwnd,msg,wParam,lParam); if (i >= 0) return(i); }
	switch (msg)
	{
		case WM_SIZE:       xres  = (lParam&65535); yres  = (lParam>>16); break;
		...
		case 0x00ff: //WM_INPUT
			...
			if (*(long *)&tbuf/*raw->header.dwType*/ != 0/*RIM_TYPEMOUSE*/) break;
#ifndef _WIN64
			u = 20;
#else
			u = 28;
#endif
			dmousx += *(long *)&tbuf[u+ 8]; //raw->data.mouse.lLastX;
			dmousy += *(long *)&tbuf[u+12]; //raw->data.mouse.lLastY;
...
int initapp (HINSTANCE hinst)
{
	...
	ghwnd = CreateWindowEx(0,prognam,prognam,WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_MAXIMIZEBOX|WS_THICKFRAME,
	...
	ShowWindow(ghwnd,SW_NORMAL); //SW_MAXIMIZE);
	UpdateWindow(ghwnd);
...
void uninitapp () { wminput_uninit(); DeleteObject(gscr.hBitmap); }
int breath ()
{
	MSG msg;
...
void print6x8 (tiletype *dd, int ox, int y, long fcol, long bcol, const char *fmt, ...)
{
	va_list arglist;
	char st[1024], *c, *v;
	int i, j, ie, x;
	...
	lp = (long *)(y*dd->p+dd->f);
	for(j=1;j<256;y++,lp=(long *)(((INT_PTR)lp)+dd->p),j+=j)
...
void drawpix (tiletype *dd, int x, int y, long col)
{
	if (((unsigned)x < (unsigned)dd->x) && ((unsigned)y < (unsigned)dd->y))
		*(long *)(dd->p*y + (x<<2) + dd->f) = col;
}

void drawhlin (tiletype *dd, int x0, int x1, int y, long col)
{
	int x;
	long *lptr;
	...
	lptr = (long *)(dd->p*y + dd->f);
	for(x=x0;x<x1;x++) lptr[x] = col;
}

void drawrectfill (tiletype *dd, int x0, int y0, int x1, int y1, long col)
{
	int x, y;
	long *lptr;
	...
	lptr = (long *)(dd->p*y0 + dd->f);
	for(y=y0;y<y1;y++,lptr=(long *)(((INT_PTR)lptr)+dd->p))
		for(x=x0;x<x1;x++) lptr[x] = col;
}

void drawline (tiletype *dd, float x0, float y0, float x1, float y1, long col)
{
	float f;
...
void drawcirc (tiletype *dd, int xc, int yc, int r, long col)
{
	int x, y, d;
...
void drawcircfill (tiletype *dd, int xc, int yc, int r, long col)
{
	int i, y;
	for(y=yc-r;y<=yc+r;y++)
	{
